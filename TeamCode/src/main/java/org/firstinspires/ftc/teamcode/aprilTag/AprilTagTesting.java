    ...
    private void telemetryAprilTag() {
        ...
        for (AprilTagDetection detection : currentDetections) {
            if (detection.metadata != null) {
                Position pose = detection.robotPose.getPosition();
                YawPitchRollAngles or = detection.robotPose.getOrientation();
                
                telemetry.addLine(String.format("\n==== (ID %d) %s", detection.id, detection.metadata.name));
                telemetry.addLine(String.format("XYZ %6.1f %6.1f %6.1f  (inch)", pose.x, pose.y, pose.z));
                telemetry.addLine(String.format("PRY %6.1f %6.1f %6.1f  (deg)", or.getPitch(), or.getRoll(), or.getYaw()));
            } else {
