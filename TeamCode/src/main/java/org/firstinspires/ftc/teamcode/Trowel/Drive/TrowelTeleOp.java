...
public class TrowelTeleOp extends OpMode {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // AUTOMATIC AIMING SYSTEM — CAMERA ONLY
    // Hold LB to engage. Uses AprilTag camera bearing directly as the
    // heading error. Filters tags by selected alliance. When no tag is
    // visible the driver has full manual control; auto-turn resumes the
    // instant a tag reappears. One PID controller, one state.
    // ══════════════════════════════════════════════════════════════════════════
    ...
    // All tunable values are defined in TrowelConfigs and can be modified
    // in real-time via the Panels Dashboard.
    // ══════════════════════════════════════════════════════════════════════════
    ...
    // What the auto-aim is currently seeing (for telemetry & control)
    ...
    // ──────────────────────────────────────────────────────────────
    // AUTO-AIM — CAMERA ONLY
    // Finds the closest alliance-appropriate AprilTag and uses its
    // bearing directly as the heading error for the PID controller.
    // When no tag is visible the driver has full manual control.
    // ──────────────────────────────────────────────────────────────

    private void updateAutoAim() {
        // ── Find best alliance tag ──
        autoAimHasTarget = false;
        autoAimBearing   = 0;
        autoAimTagId     = -1;
        autoAimTagRange  = 0;
        ...
        if (robot.isVisionEnabled()) {
            ...
            for (AprilTagDetection det : detections) {
                ...
                // Alliance filter — skip tags that don't belong to our alliance
        ...
        if (!autoAimHasTarget) {
            // No tag visible — give the driver full manual control so
            // they can turn to find a tag. Auto-aim stays ACTIVE and
            // will resume the instant a tag is detected.
            updateManualDrive();
            return;
        }

        // ── Camera bearing IS the heading error ──
    ...
    /**
     * Applies mecanum drive with auto-aim rotation while allowing driver
     * forward/strafe input. Rotation is fully controlled by the PID.
     */
    ...
    // ──────────────────────────────────────────────────────────────
    // TURN PID CONTROLLER  (Pedro Pathing constants)
    // Input:  heading error in degrees (positive → turn right)
    // Output: motor power  [-MAX_TURN_POWER … +MAX_TURN_POWER]
    // ──────────────────────────────────────────────────────────────
    ...
    private void updateTelemetry() {
        ...
        if (autoAimState == AutoAimState.DISABLED) {
            ...
            telemetry.addData("Status", "🔒 LOCKED  bearing %.1f°", autoAimBearing);
            telemetry.addData("  Tag", "#%d  %.1f\" away", autoAimTagId, autoAimTagRange);
        } else {
            telemetry.addData("Status", "🎯 AIMING  bearing %.1f°", autoAimBearing);
            telemetry.addData("  Tag", "#%d  %.1f\" away", autoAimTagId, autoAimTagRange);
        }
        telemetry.addLine("");
